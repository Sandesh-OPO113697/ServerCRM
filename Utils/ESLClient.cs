...
namespace ServerCRM.Utils
{
    public class ESLClient : IDisposable
    {
        ...
        public async Task ConnectAsync(CancellationToken ct = default)
        {
            ...
            var authReply = await ReadFrameAsync(ct);
            if (authReply == null || !authReply.Contains("Reply-Text: +OK"))
                throw new Exception("ESL auth failed: " + authReply);
        }

        public async Task StartEventListenerAsync(CancellationToken ct = default)
        {
            ...
            _ = Task.Run(async () =>
            {
                ...
                while (!ct.IsCancellationRequested && IsConnected)
                {
                    try
                    {
                        var read = await _stream!.ReadAsync(buff, 0, buff.Length, ct);
                        if (read <= 0) break;
                        ...
                        while (true)
                        {
                            ...
                            try { OnEventReceived?.Invoke(frame); } catch { /* swallow */ }
                        }
                    }
                    catch
                    {
        ...
        public async Task<string> SendCommandAsync(string cmd, CancellationToken ct = default)
        {
            ...
            var reply = await ReadFrameAsync(ct);
            return reply ?? string.Empty;
        }

        private async Task SendRawAsync(string data, CancellationToken ct = default)
        {
        ...
        private async Task<string?> ReadFrameAsync(CancellationToken ct = default)
        {
            if (_stream == null) return null;
            var buffer = new byte[8192];

            var sw = Stopwatch.StartNew();
            while (!ct.IsCancellationRequested && sw.Elapsed < TimeSpan.FromSeconds(5))
            {
                try
                {
